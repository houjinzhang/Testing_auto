...
# ===== 基础配置 =====
DEVICE_SERIAL = 'A59GVB5A10000041'
...
class MainApp(QWidget):
    ...
    # === 录屏相关 begin ===
    def start_record_screen(self, save_path_on_phone="/sdcard/test_loop_record.mp4"):
        try:
            self.record_screen_proc = subprocess.Popen(
                ["adb", "-s", DEVICE_SERIAL, "shell", "screenrecord", save_path_on_phone],
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE
            )
            self.append_log(f"🎥 开始录屏: {save_path_on_phone}")
        except Exception as e:
            self.append_log(f"⚠️ 录屏启动失败: {e}")

    def stop_record_screen(self):
        if hasattr(self, 'record_screen_proc') and self.record_screen_proc:
            try:
                self.record_screen_proc.terminate()
                self.append_log(f"🛑 录屏已手动终止")
            except Exception as e:
                self.append_log(f"⚠️ 停止录屏出错: {e}")

    def pull_record_file(self, phone_path="/sdcard/test_loop_record.mp4", pc_path="record_last.mp4"):
        try:
            out_path = os.path.join(REPORT_DIR, pc_path)
            result = subprocess.run(["adb", "-s", DEVICE_SERIAL, "pull", phone_path, out_path], capture_output=True)
            if result.returncode == 0:
                self.append_log(f"🎥 录屏已保存至: {out_path}")
            else:
                self.append_log(f"⚠️ adb pull 录屏时出错: {result.stderr.decode()}")
            # 删除手机端的录屏文件
            subprocess.run(["adb", "-s", DEVICE_SERIAL, "shell", "rm", phone_path])
        except Exception as e:
            self.append_log(f"⚠️ 拉取录屏失败: {e}")
    # === 录屏相关 end ===
    ...
    def run_test(self):
        try:
            ...
            for i in range(100):
                ...
                # 在第90轮开始录屏
                if i == 89:
                    self.append_log("🎥 第90次开始录屏...")
                    rec_name = f"/sdcard/loop_90_100_{datetime.datetime.now().strftime('%H%M%S')}.mp4"
                    self.rec_name_on_phone = rec_name
                    self.start_record_screen(save_path_on_phone=rec_name)
                    start_recorded = True
                ...
                w3c_pinch_zoom(self.driver, (469, 721), (521, 681), (263, 812), (242, 861))
                self.log_step("循环-放大图片")
                time.sleep(2)

                w3c_pinch_zoom(self.driver, (521, 680), (469, 721), (242, 861), (263, 812))
                self.log_step("循环-缩小图片")
                time.sleep(2)

                w3c_swipe(self.driver, (275, 823), (422, 894), duration=1500)
                self.log_step("循环-单指滑动")
                self.safe_screenshot(f"第{i+1}次-滑动后截图")
                time.sleep(1.0)

                self.driver.tap([(601, 1295)])
                self.log_step("循环-点击恢复")
                self.safe_screenshot(f"第{i+1}次-恢复后截图")
                time.sleep(0.5)

                # 到100次（第99轮）时停止录屏并pull
                if i == 99 and start_recorded:
                    self.append_log("🛑 第100次录屏结束，正在保存录屏文件...")
                    self.stop_record_screen()
                    time.sleep(3)
                    rec_pc_name = f"record_loop_90_100_{datetime.datetime.now().strftime('%H%M%S')}.mp4"
                    self.pull_record_file(phone_path=self.rec_name_on_phone, pc_path=rec_pc_name)
