...
class MainApp(QWidget):
    ...
    def run_test(self):
        try:
            ...
            if self.execute_step('input tap 653 2525', "点击AI编辑", sleep_after=1.5): return
            if self.execute_step('input tap 576 2482', "点击裁剪", sleep_after=1.5): return

            # 旋转 100 次
            for i in range(100):
                if self.execute_step('input tap 110 2272', f"旋转第 {i + 1} 次", sleep_after=1): return

            # 镜像 100 次
            for i in range(100):
                if self.execute_step('input tap 1131 2266', f"镜像第 {i + 1} 次", sleep_after=1): return

        except Exception as e:
            self.comm.log_signal.emit(f"❌ 出错: {e}")
