...
class MainApp(QWidget):
    ...
    def run_test(self):
        try:
            ...
            if self.execute_step('input tap 767 1688', "进入图库app", 1.5): return

            if self.execute_step('input tap 158 2530', "进入图库后点击指定区域", 1.0): return

            if self.execute_step('input tap 212 1185', "选择第一张照片", 1.5): return
            if self.execute_step('input tap 984 278', "点击右上角", 1.0): return
            if self.execute_step('input tap 841 612', "点击慢放", 1.0): return

            # 慢放循环
            for i in range(100):
                self.comm.log_signal.emit(f"--- 第 {i + 1}/100 次慢放循环 ---")
                self.execute_step('input tap 619 2529', "生成", 1)
                self.execute_step('input tap 619 2529', "取消", 1)

            # 点击运动幻影
            self.execute_step('input tap 800 2140', "点击运动幻影", 1.0)

            # 幻影循环
            for i in range(100):
                if not self.is_running: break
                self.comm.log_signal.emit(f"--- 第 {i + 1}/100 次幻影循环 ---")
                self.execute_step('input tap 619 2529', "生成", 1.5)
                self.execute_step('input tap 619 2529', "取消", 1)

            # 点击运动分身
            self.execute_step('input tap 1070 2140', "点击运动分身", 1.0)

            # 分身循环
            for i in range(100):
                self.comm.log_signal.emit(f"--- 第 {i + 1}/100 次分身循环 ---")
                self.execute_step('input tap 619 2529', "生成", 1)
                self.execute_step('input tap 619 2529', "取消", 1)
