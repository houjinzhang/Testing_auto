...
# 1. 基础配置 和 ADB 命令执行器
# =============================================================================
DEVICE_SERIAL = 'A22C015925000107'  # 修改为你的设备序列号
...
class MainApp(QWidget):
    def __init__(self):
        ...
        self.setWindowTitle("抠图恢复压测100次")
    ...
    def run_test(self):
        try:
            ...
            if self.execute_step('input swipe 244 1235 244 1235 1000', "长按第一张1秒", 1.0): return
            if self.execute_step('input tap 620 1199', "选中第二张", 0.5): return
            if self.execute_step('input tap 1063 1210', "选中第三张", 0.5): return
            if self.execute_step('input tap 233 1649', "选中第四张", 0.5): return
            if self.execute_step('input tap 627 2489', "点击创作", 1.0): return
            if self.execute_step('input tap 742 2245', "点击拼图", 1.0): return
            if self.execute_step('input tap 495 2128', "选择四方格", 1.0): return
            if self.execute_step('input tap 245 807', "循环-选第一张", 1.0): return
            if self.execute_step('input tap 181 1642', "循环-点击扣他", 1.0): return
            if self.execute_step('input tap 843 1479', "循环-取消", 1.0): return

            for i in range(50):
                if not self.is_running: break
                self.comm.log_signal.emit(f"--- 第 {i + 1}/50 次循环 ---")
                if self.execute_step('input tap 245 807', "循环-选第一张", 1.0): break
                if self.execute_step('input tap 181 1642', "循环-点击扣他", 1.0): break
                if self.execute_step('input tap 843 1479', "循环-取消", 1.0): break


            self.take_screenshot("抠图恢复100次后截图")
        except Exception as e:
